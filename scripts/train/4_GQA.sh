...
torchrun --nproc_per_node=${NPROC} -m training.train \
    --train_data "${DATA_ROOT}/GQA/train_data.jsonl" \
    --ckpt "${OUTPUT_ROOT}/ImageNet/${RUN_NAME}/final_merged_model" \
    --base_model "${BASE_MODEL}" \
    --ds training/ds_config.json \
    --output_dir "${OUTPUT_ROOT}/GQA/${RUN_NAME}" \
    ...
    --save_strategy steps --save_steps 100000 --eval_steps 100 \
    --name "GQA-${RUN_NAME}" \
    $(wandb_args)
