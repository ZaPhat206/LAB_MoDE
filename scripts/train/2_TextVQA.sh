...
torchrun --nproc_per_node=${NPROC} -m training.train \
    --train_data "${DATA_ROOT}/TextVQA/train_data.jsonl" \
    --ckpt "${OUTPUT_ROOT}/ScienceQA/${RUN_NAME}/final_merged_model" \
    --base_model "${BASE_MODEL}" \
    --ds training/ds_config.json \
    --output_dir "${OUTPUT_ROOT}/TextVQA/${RUN_NAME}" \
