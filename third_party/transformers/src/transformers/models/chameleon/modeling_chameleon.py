...
class ChameleonAttention(nn.Module):
    ...
    def __init__(self, config: ChameleonConfig, layer_idx: Optional[int] = None):
        ...
        if config.g_prompt_length is not None: 
            # adapter embedding layer
            self.adapter_wte = nn.Embedding(config.g_prompt_length, self.hidden_size)
            ...
            self.gating_factor_g_prompt = torch.nn.Parameter(torch.zeros(1, self.num_heads, 1, 1))
            self.gating_factor_e_prompt = torch.nn.Parameter(torch.zeros(1, self.num_heads, 1, 1))
...
class ChameleonSdpaAttention(ChameleonAttention):
    ...
    def forward(
        ...
        use_cache: bool = False,
        cache_position: Optional[torch.LongTensor] = None,
        g_prompt_length: int = -1,
        e_prompt: torch.Tensor = None,
        token_type_ids=None,
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        # print(f"sdpa prompt: {prompt}")
        ...
        #     self.key_ = query_states
        #     # prefix key, value
        #     prompt = prompt.permute(1, 0, 3, 2, 4).contiguous() # 2, bsz, num_heads, prompt_length, channel // num_heads
        #     key_prefix = prompt[0] # bsz, num_heads, prompt_length, embed_dim // num_heads
        #     value_prefix = prompt[1] # bsz, num_heads, prompt_length, embed_dim // num_heads
        #     expected_shape = (bsz, self.num_heads, channel // self.num_heads)
            
        #     assert (key_prefix.shape[0], key_prefix.shape[1], key_prefix.shape[3]) == expected_shape, f'key_prefix.shape: {key_prefix.shape} not match k.shape: {key_states.shape}'
        #     assert (value_prefix.shape[0], value_prefix.shape[1], value_prefix.shape[3]) == expected_shape, f'value_prefix.shape: {value_prefix.shape} not match v.shape: {value_states.shape}'
        ...
        # g_prompt
        if g_prompt_length > 0:
            prefix = self.adapter_wte.weight.reshape(1, g_prompt_length, self.hidden_size)
            aT = prefix.size(1)
            ak = self.k_proj(prefix)  # (1, aT, C)
            av = self.v_proj(prefix)  # (1, aT, C)
            # Reshape and expand to match batch and head dimensions: (B, n_head, aT, head_size)
            ak = ak.view(1, aT, self.num_heads, self.head_dim).repeat(bsz, 1, 1, 1).transpose(1, 2)
            av = av.view(1, aT, self.num_heads, self.head_dim).repeat(bsz, 1, 1, 1).transpose(1, 2)

            amask = torch.ones(query_states.shape[-2], ak.shape[-2], dtype=torch.bool, device=hidden_states.device)
            ay = F.scaled_dot_product_attention(query_states, ak, av, attn_mask=amask, dropout_p=0.0, is_causal=False)

            attn_output = attn_output + ay * self.gating_factor_g_prompt
        
        # e_prompt
        if e_prompt is not None:
            # input e_prompt shape: [B, 2, top_k * e_prompt_length, num_heads, embed_dim // num_heads]
            total_e_prompt_length = e_prompt.size(2)
            e_prefix = e_prompt[:,0,:,:].reshape(bsz, total_e_prompt_length, self.hidden_size)
            aT = e_prefix.size(1)
            ak = self.k_proj(e_prefix) # (B, sT, C)
            av = self.v_proj(e_prefix) # (B, sT, C)
            # Reshape and expand to match batch and head dimensions: (B, n_head, aT, head_size)
            ak = ak.view(bsz, aT, self.num_heads, self.head_dim).transpose(1, 2)
            av = av.view(bsz, aT, self.num_heads, self.head_dim).transpose(1, 2)
            amask = torch.ones(query_states.shape[-2], ak.shape[-2], dtype=torch.bool, device=hidden_states.device)
            ay = F.scaled_dot_product_attention(query_states, ak, av, attn_mask=amask, dropout_p=0.0, is_causal=False)
            attn_output = attn_output + ay * self.gating_factor_e_prompt
...
class ChameleonModel(ChameleonPreTrainedModel):
    ...
    def __init__(self, config: ChameleonConfig):
        ...
        self.use_g_prompt = config.use_g_prompt
        self.g_prompt_layer_idx = config.g_prompt_layer_idx
        ...
        self.use_prefix_tune_for_g_prompt = config.use_prefix_tune_for_g_prompt
        self.g_prompt_length = config.g_prompt_length
        
        self.use_e_prompt = config.use_e_prompt
        ...
        self.use_prefix_tune_for_e_prompt = config.use_prefix_tune_for_e_prompt

        if not self.use_prefix_tune_for_g_prompt:
            self.use_g_prompt = False
            self.g_prompt_layer_idx = []

        # if config.use_g_prompt and config.g_prompt_length is not None and len(config.g_prompt_layer_idx) != 0:
        #     if not config.use_prefix_tune_for_g_prompt:
        #         g_prompt_shape=(num_g_prompt, config.g_prompt_length, config.hidden_size)
        ...
        #     else:
        #         if config.same_key_value:
        #             g_prompt_shape=(num_g_prompt, 1, config.g_prompt_length, config.num_attention_heads, config.hidden_size // config.num_attention_heads)
        #             if config.prompt_init == 'zero':
        #                 self.g_prompt = nn.Parameter(torch.zeros(g_prompt_shape))
        #             elif config.prompt_init == 'uniform':
        #                 self.g_prompt = nn.Parameter(torch.randn(g_prompt_shape))
        ...
        #         else:
        #             g_prompt_shape=(num_g_prompt, 2, config.g_prompt_length, config.num_attention_heads, config.hidden_size // config.num_attention_heads)
        #             if config.prompt_init == 'zero':
        #                 self.g_prompt = nn.Parameter(torch.zeros(g_prompt_shape))
        #             elif config.prompt_init == 'uniform':
        #                 self.g_prompt = nn.Parameter(torch.randn(g_prompt_shape))
        ...
        if config.use_e_prompt and config.e_prompt_layer_idx is not None:
            # Prompt-tuning baseline. MoDE never enables this (use_e_prompt defaults to
            # False), so the dependency is imported lazily and is not required to run MoDE.
            from training.dualprompt_pgp.prompt import EPrompt

            self.e_prompt = EPrompt(length=config.prompt_length, embed_dim=config.hidden_size, prompt_init=config.prompt_init,
                    prompt_pool=config.prompt_pool, prompt_key=config.prompt_key, pool_size=config.pool_size, top_k=config.top_k_prompt, batchwise_prompt=config.batchwise_prompt,
                    prompt_key_init=config.prompt_key_init, num_layers=num_e_prompt, use_prefix_tune_for_e_prompt=config.use_prefix_tune_for_e_prompt,
                    num_heads=config.num_attention_heads, same_key_value=config.same_key_value)

        self.total_prompt_len = 0
        if self.prompt_pool:
            if not self.use_prefix_tune_for_g_prompt:
                self.total_prompt_len += config.g_prompt_length * len(self.g_prompt_layer_idx)
            if not self.use_prefix_tune_for_e_prompt:
                self.total_prompt_len += config.prompt_length * config.top_k_prompt * len(self.e_prompt_layer_idx)
    ...
    def forward(
        ...
        if self.use_g_prompt or self.use_e_prompt:
            if self.use_prompt_mask and self.training and self.use_e_prompt:
            ...
            if self.use_e_prompt:
                res = self.e_prompt(hidden_states, prompt_mask=prompt_mask, cls_features=cls_features)
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if self.use_g_prompt and i in self.g_prompt_layer_idx:
                    # if self.use_prefix_tune_for_g_prompt:
                    #     g_prompt_counter += 1
                    #     # Prefix tunning, [B, 2, g_prompt_length, num_heads, embed_dim // num_heads]
                    #     idx = torch.tensor([g_prompt_counter] * hidden_states.shape[0]).to(hidden_states.device)
                    #     g_prompt = self.g_prompt[idx]
                    # else:
                    #     g_prompt=None
                    # g_causal_mask = self._update_causal_mask(
                    #     attention_mask, inputs_embeds, cache_position, past_key_values, output_attentions, g_prompt.shape[-3]
                    # )
                    ...
                    # self.g_prefix_feature[i] = {"key": decoder_layer.self_attn.key_}
                elif self.use_e_prompt and i in self.e_prompt_layer_idx:
                    e_prompt_counter += 1
                    if self.use_prefix_tune_for_e_prompt:
                        # Prefix tuning, [B, 2, top_k * e_prompt_length, num_heads, embed_dim // num_heads]
                        # e_causal_mask = self._update_causal_mask(
                        #     attention_mask, inputs_embeds, cache_position, past_key_values, output_attentions, e_prompt[e_prompt_counter].shape[-3]
                        # )
                        layer_outputs = decoder_layer(
                            hidden_states,
                            attention_mask=causal_mask,
                            ...
                            use_cache=use_cache,
                            cache_position=cache_position,
                            e_prompt=e_prompt[e_prompt_counter],
                        )
                        # self.e_prefix_feature[i] = {"key": decoder_layer.self_attn.key_}
                    else:
    ...
    def _update_causal_mask(
        ...
        if using_static_cache:
            ...
            # )
            expected_target_length = past_seen_tokens + sequence_length + 1 + prompt_length
            if isinstance(attention_mask, torch.Tensor):
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        **kwargs,
        ...
        # print(f"4D prompt length in causal mask: {prompt_length}")
        """
