...
class ChameleonProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        **kwargs: Unpack[ChameleonProcessorKwargs],
        ...
        # --- Generate token_type_ids based on the image boundaries ---
        # Convert the special tokens to their corresponding ids.
        image_start_id = self.tokenizer.convert_tokens_to_ids(self.image_start_token)
        image_end_id = self.tokenizer.convert_tokens_to_ids(self.image_end_token)

        # We'll compute modality ids so that tokens in an image region get a 1 and all others get a 0.
        # Here we assume that data["input_ids"] is a tensor if return_tensors is specified.
            ...
            for seq in data["input_ids"]:
                ...
                for token in seq.tolist():
                    if token == image_start_id:
                        ...
                        mod_seq.append(current_mod)
                token_type_ids.append(mod_seq)
            data["token_type_ids"] = torch.tensor(token_type_ids, dtype=torch.long)
        else:
            # For other tensor types (or if the tokenizer returns lists), process accordingly.
            token_type_ids = []
            for seq in data["input_ids"]:
                current_mod = 0
                mod_seq = []
