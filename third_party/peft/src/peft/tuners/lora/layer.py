...
class MMoELoraLayer:
    adapter_layer_names = ("lora_A_text0", "lora_B_text0", "lora_route_text", "lora_A_image0", "lora_B_image0") #, "lora_route_image")
    def __init__(
        self,
        r: int,
        lora_alpha: int,
...
class MMoELoRALinear(nn.Linear, MMoELoraLayer):
    ...
    def __init__(
        ...
        r: int = 0,
        lora_alpha: int = 1,
        lora_nums: int = 2, # number of expert LoRA modules. 
        blc_alpha: float = 0.0,
        ...
        **kwargs,
        ...
        # Actual trainable parameters
        if r > 0:
            # self.lora_route_image = nn.Linear(in_features, self.lora_num, bias=False)
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, "lora_A_image0"):
            ...
            # nn.init.kaiming_uniform_(self.lora_route_image.weight, a=math.sqrt(5))

    def train(self, mode: bool = True):
        nn.Linear.train(self, mode)
        self.lora_route_text.train(mode)
        # self.lora_route_image.train(mode)
        for i in range(self.lora_num):
    ...
    def forward(self, x: torch.Tensor, modality_ids: torch.Tensor = None, return_aux: bool = False, task_types=None):
        ...
        if self.disable_adapters:
            ...
            if modality_ids is not None:
                # We assume x is of shape [B, ..., in_features] and modality_ids is [B, ...].
                # Flatten x and modality_ids so we can process tokens independently.
                orig_shape = x.shape[:-1]
                x = x.view(-1, x.shape[-1])
                modality_ids_flat = modality_ids.view(-1)

                # text_out = torch.zeros(x_flat.size(0), self.out_features, dtype=x_flat.dtype, device=x_flat.device)
                # image_out = torch.zeros(x_flat.size(0), self.out_features, dtype=x_flat.dtype, device=x_flat.device)
                ...
                if text_mask.any():
                    ...
                    for i in range(self.lora_num):
                        ...
                        # temp_text_out = temp_text_out + expert_weight * expert_out * self.scaling
                        vl_out[text_mask] += expert_weight * expert_out * self.scaling #temp_text_out

                # Process image tokens.
                image_mask = modality_ids_flat == 1
                if image_mask.any():
                    x_image = x[image_mask]
                    # x_image_float = x_image.to(torch.float32)
                    # lora_route_image_weight = self.lora_route_image.weight.to(torch.float32)
                    # lora_route_image_bias = self.lora_route_image.bias.to(torch.float32) if self.lora_route_image.bias is not None else None
                    # image_route_weight = torch.nn.functional.linear(x_image_float, lora_route_image_weight, lora_route_image_bias)
                    # image_route_weight = nn.functional.softmax(image_route_weight, dim=-1)
                    # image_route_weight = image_route_weight.to(x.dtype)
                    dropped_x_image = self.lora_dropout(x_image).to(x.dtype)
                    # temp_image_out = 0
                    for i in range(1):
                        # expert_weight = torch.unsqueeze(image_route_weight[:,i], -1)
                        expert_out = getattr(self, f"lora_A_image{i}")(dropped_x_image)
                        expert_out = getattr(self, f"lora_B_image{i}")(expert_out)
                        # temp_image_out = temp_image_out + expert_weight * expert_out * self.scaling
                        vl_out[image_mask] += expert_out * self.scaling #temp_image_out

                result = result + vl_out.view(*orig_shape, self.out_features) # (text_out + image_out).view(*orig_shape, self.out_features)
            else:
